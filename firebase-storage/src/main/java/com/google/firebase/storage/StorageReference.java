...
public class StorageReference implements Comparable<StorageReference> {
  ...
  /**
   * List up to `maxResults` items (files) and prefixes (folders) under this storage reference.
   *
   * <p>"/" is treated as a path delimiter. Firebase storage does not support invalid object paths
   * that ends with "/" or contains two consecutive "//". All invalid objects in GCS will be
   * filtered.
   *
   * @param maxResults The maximum number of results to return in a single page. Must be greater
   *     than 0 and at most 1000.
   * @return A a {@link Task} that returns the requested results.
   */
  @NonNull
  @PublicApi
  ...
  /**
   * Resumes a previous call to {@link #list(int)}, starting after a previous result. Returns the
   * next set of items (files) and prefixes (folders) under this storage reference.
   *
   * <p>"/" is treated as a path delimiter. Firebase storage does not support invalid object paths
   * that ends with "/" or contains two consecutive "//". All invalid objects in GCS will be
   * filtered.
  ...
  public Task<ListResult> list(int maxResults, @NonNull String pageToken) {
    ...
    Preconditions.checkArgument(
        pageToken != null,
        "pageTokens must be non-null in order to resume a previous list() operation");
    TaskCompletionSource<ListResult> pendingResult = new TaskCompletionSource<>();
    StorageTaskScheduler.getInstance()
   ...
   * default pagination size is 1000. Consistency of the result can't be guaranteed if objects are
   * inserted or removed while this operation is executing.
   *
   * @throws OutOfMemoryError If there are too many items under this location.
   * @return A {@link Task} that returns all the items and prefixes under the current storage
   *     reference.
   */
  @NonNull
  @PublicApi
  public Task<ListResult> listAll() {
        ...
        new Continuation<ListResult, Task<Void>>() {
          ...
          public Task<Void> then(@NonNull Task<ListResult> currentPage) {
            ...
            if (result.getPageToken() != null) {
              ...
              nextPage.continueWithTask(this);
            } else {
    ...
    list.continueWithTask(continuation);
